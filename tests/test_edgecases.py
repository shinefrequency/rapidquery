...
class TestAdaptedValueEdgeCases:
    ...
    def test_decimal_precision_edge_cases(self):
        ...
        large_decimal = _lib.AdaptedValue(
            decimal.Decimal("99999999999999999999.99"), _lib.DecimalType((22, 2))
        )
...
class TestTableDefinitionEdgeCases:
    ...
    def test_foreign_key_to_nonexistent_column(self):
        ...
        table = _lib.Table(
            ...
            columns=[
                _lib.Column("id", _lib.IntegerType()),
                _lib.Column("user_id", _lib.IntegerType()),
            ],
            foreign_keys=[
                _lib.ForeignKey(
...
class TestInsertEdgeCases:
    ...
    def test_insert_multiple_values_calls(self):
        ...
        insert = (
            _lib.Insert().into("users").columns("id", "name").values(1, "John").values(2, "Jane")
        )
        ...
        alter = _lib.AlterTable(
            ...
            options=[
                _lib.AlterTableAddColumnOption(
                    _lib.Column("temp", _lib.IntegerType()), if_not_exists=False
                ),
...
class TestSQLInjectionPrevention:
    ...
    def test_string_with_quotes_in_where(self):
        ...
        delete = _lib.Delete().from_table("users").where(_lib.Expr.col("name") == "O'Brien")
        sql, params = delete.build("postgresql")
    ...
    def test_comment_injection_attempt(self):
        ...
        insert = _lib.Insert().into("users").values(name="'; DROP TABLE users; --")
        sql, params = insert.build("sqlite")
    ...
    def test_union_injection_attempt(self):
        ...
        delete = (
            _lib.Delete()
            .from_table("users")
            .where(_lib.Expr.col("id") == "1 UNION SELECT * FROM passwords")
        )
        sql, params = delete.build("postgresql")
...
class TestWhereChaining:
    def test_select(self):
        query = _lib.Select(_lib.ASTERISK).from_table("users").where(_lib.Expr.col("id") > 10).where(_lib.Expr.col("id") < 20)

        assert "AND" in query.to_sql("postgresql")

    def test_delete(self):
        query = _lib.Delete().from_table("users").where(_lib.Expr.col("id") > 10).where(_lib.Expr.col("id") < 20)
